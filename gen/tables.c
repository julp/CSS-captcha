...
void generate_tables_for_elixir(FILE *fp, DArray *da, size_t offsets[][_UNICODE_VERSIONS_COUNT + 1 /* for start offsets */])
{
    ...
    fputs("# /!\\ DO NOT EDIT THIS FILE, IT WAS AUTO-GENERATED /!\\\n\n", fp);
    fputs("defmodule ExCSSCaptcha.Table do\n\n", fp);
    fputs("  @type unicode_version :: ", fp);
    ...
    generic_generate_table(fp, da, "\n\n  @table <<", "  >>\n", "::32");
    fputs("  @spec offset(digit :: char, version :: unicode_version) :: {non_neg_integer, non_neg_integer}\n", fp);
    for (i = 0; i < _TABLE_COUNT; i++) {
        for (v = 0; v < _UNICODE_VERSIONS_COUNT; v++) {
            size_t e;
    ...
    fputs("\n  @spec map(digit :: char, version :: unicode_version) :: char\n", fp);
    fputs("  def map(digit, version) do\n", fp);
    fputs("    {s, e} = offset(digit, version)\n", fp);
    fputs("    offset = ExCSSCaptcha.random(s, e)\n", fp);
    fputs("    <<cp::32>> = binary_part(@table, offset * 4, 4)\n", fp);
    fputs("    cp\n", fp);
    fputs("  end\n", fp);
    fputs("end\n", fp);
}
